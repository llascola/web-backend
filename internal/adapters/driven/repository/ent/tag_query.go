...
func (_q *TagQuery) QueryPosts() *BlogPostQuery {
...
func (_q *TagQuery) WithPosts(opts ...func(*BlogPostQuery)) *TagQuery {
	query := (&BlogPostClient{config: _q.config}).Query()
...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := _q.withPosts; query != nil {
		if err := _q.loadPosts(ctx, query, nodes,
			func(n *Tag) { n.Edges.Posts = []*BlogPost{} },
			func(n *Tag, e *BlogPost) { n.Edges.Posts = append(n.Edges.Posts, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TagQuery) loadPosts(ctx context.Context, query *BlogPostQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *BlogPost)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(blogpost.FieldID), joinT.C(tag.PostsPrimaryKey[0]))
